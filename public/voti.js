document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (materie.length && voti.length) {
      ...
      votiDiv.innerHTML = '<div class="empty-voti">Nessun voto disponibile al momento.</div>';
...
function handleMateriaChange(materia, voti) {
  ...
  if (!materia) {
    votiDiv.innerHTML = '<div class="empty-voti">Seleziona una materia per vedere i voti.</div>';
    renderMedia(0, "Nessun periodo");
    return;
  }
  ...
  if (votiPrimoPeriodo.length > 0) {
    ...
    votiDiv.innerHTML = '<div class="empty-voti">Nessun voto disponibile per questa materia.</div>';
...
function renderActualVoti(voti, periodoLabel = "Periodo", materia = "") {
  ...
  if (!voti.length) {
    votiDiv.innerHTML = '<div class="empty-voti">Nessun voto disponibile per questa selezione.</div>';
  }
...
function openEntryModal(voto) {
  ...
  const teacherToShow = voto.authorName || voto.teacherName || voto.insegnante || "Docente non disponibile";
  if (modalTeacher) {
  ...
  const parts = [];
  if (voto.displayValue || voto.voto || voto.grade) parts.push(`Voto: ${voto.displayValue || voto.voto || voto.grade}`);
  if (voto.evtDate) {
    try { parts.push(`Data: ${new Date(voto.evtDate).toLocaleDateString("it-IT")}`); } catch {}
  }
  ...
  if (modalText) modalText.innerHTML = escapeHTML(fullText).replace(/\n/g, "<br>");
  ...
  function onOverlayClick(e) { if (e.target === entryModal) closeModal(); }
...
function goToHome() { window.location.href = "/dashboard.html"; }
function goToAssenze() { window.location.href = "/assenze.html"; }
